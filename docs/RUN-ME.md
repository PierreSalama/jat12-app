...
| **App installer** | `app/release/JAT-12-Setup-12.0.0.exe` | double-click to install (per-user, no admin) |

You don't strictly need the installer: **the extension finds the app itself.** Its popup probes the app on `127.0.0.1:7845`; if the app is running it pairs automatically, and if it isn't it shows a **"Download the JAT 12 app"** button that goes to the latest GitHub release. After the first release, the app also **self-updates** (electron-updater, public repo → no token needed).

## First-run / cutover sequence

1. **Install + launch the app** (`JAT-12-Setup-12.0.0.exe`). It opens the Aurora window and starts the brain on `127.0.0.1:7845`.
...
7. Once v12 is doing the work, **leave v11 shut down.**

## Dev mode (no installer)

```bash
npm install
npm run dev        # launches the app with the dev identity (port 7846, userData jat12-app-dev)
...
Bump the version in the `package.json`s (keep them in lockstep — `npm run gates` checks), then tag `v12.x.y` and push. `.github/workflows/release.yml` builds the installer on Windows CI and publishes it + `latest.yml` to the GitHub release; installed apps auto-update from there.
...
- **Needs your machine/click (can't be done from CI):** the first live LinkedIn apply in a real Chrome, the Gmail OAuth consent, and the actual v11→v12 import (because v11 must be quit first).
