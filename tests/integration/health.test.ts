...
describe('/health', () => {
  ...
  it('reports liveness with the migrated schema version (in-process)', async () => {
    const app = createApp({ db, version: '12.0.0', startedAt: Date.now() - 5, dev: true });
  ...
  it('serves over a real loopback socket — the M0 boot spine end-to-end', async () => {
    ...
    try {
      running = await startServer({ db, version: '12.0.0', startedAt: Date.now(), dev: true }, 0); // ephemeral port
