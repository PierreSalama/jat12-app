...
//     ('jat12.reconnect', 0.5min) re-opens a dead socket. Eviction is NORMAL — resume-by-classify (app).
//
// It holds ZERO adapter/apply logic. Every "decision" here is a transport/tab-lifecycle fact.
import { PORTS, IDENTITY, PROTOCOL_VERSION } from '@jat12/shared/constants';
import type { TypedEnvelope, ExtEvent, PageSnapshot, CmdResult } from '@jat12/shared/protocol';
...
const RECONNECT_ALARM = 'jat12.reconnect';
const APPLY_WINDOW_KEY = 'jat12.applyWindowId';
const TOKEN_KEY = 'jat12Token'; // chrome.storage.local
...
// ---------------------------------------------------------------------------
// content Port: per-tab chrome.runtime Port named 'jat12.page'. The content
...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name !== 'jat12.page') return;
