// JAT 12 renderer — plain ES module, no framework. Wears v11's Atelier look (styles.css copied
// verbatim) but is a FRESH renderer wired to v12's REAL loopback REST API (snake_case DTOs, the
// X-JAT12-Token pairing header). Every control here maps to a real v12 route — features v11 had that
// v12 does not (AI composer, taught procedures, job discovery, document upload) are absent, not faked.
...
const LS_THEME = 'jat12.theme';

// ---------------------------------------------------------------------------
// bootstrap config + pairing token
// ---------------------------------------------------------------------------
const state = {
  base: 'http://127.0.0.1:7845',
  token: null,
  version: '',
  online: false,
  applying: false,
...
/** Resolve {base, token}. Preferred: the preload bridge (window.jat12.config). Fallback (dev in a
 *  browser): probe the two candidate loopback ports for the loopback-trusted pairing token. */
async function bootstrap() {
  if (window.jat12?.config) {
    try {
...
// muted line reused wherever a v11 capability is not yet in v12
const DISCOVERY_NOTE = 'Job discovery arrives in a later v12 update — use the Import wizard or the Track button in the extension to feed the queue.';
...
function errorView(e) {
  ...
  $('.empty-sub', v).textContent = (e?.message ? e.message + ' — ' : '') + 'Start JAT 12, then retry.';
...
function renderNotConnected() {
  ...
  const v = el(`<div><div class="empty">
    ...
    <div class="empty-sub">The desktop app could not hand over its access token. Restart JAT 12; if this persists, check the logs.</div>
    <div class="mt"><button class="btn primary" data-retry>Retry</button></div>
  </div></div>`);
...
route('/applications', async () => {
  const status = sessionStorage.getItem('jat12.appStatus') || 'all';
  ...
  $('[data-filter]', v).addEventListener('change', (e) => { sessionStorage.setItem('jat12.appStatus', e.target.value); navigate(); });
...
route('/documents', async () => {
  ...
  const v = el(`<div>
    ...
    <div class="form-hint muted" style="margin-bottom:12px">Uploading documents from the dashboard isn't wired in v12 yet — documents arrive via the v11 import. This is a read-only view.</div>
...
route('/settings', async () => {
  ...
  $('[data-export]', v).addEventListener('click', async () => {
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'jat12-export.json'; a.click();
