...
import type { Cmd, CmdResult, ExtEvent, PageSnapshot } from '@jat12/shared/protocol';
import { Envelope, ExtEvent as ExtEventSchema } from '@jat12/shared/protocol';
import { IDENTITY } from '@jat12/shared/constants';
...
export interface WsGatewayOptions {
  /** the pairing token; the upgrade must present it via ?token= or the X-JAT12-Token header. */
  token: string;
  log?: (msg: string) => void;
...
function headerToken(req: IncomingMessage): string | undefined {
  const raw = req.headers['x-jat12-token'];
