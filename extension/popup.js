// JAT v12 popup — finds the desktop app on the loopback, pairs (stores the token the SW uses),
...
(function () {
  ...
  var RELEASES_PAGE = 'https://github.com/PierreSalama/jat12-app/releases/latest';
  var RELEASES_API = 'https://api.github.com/repos/PierreSalama/jat12-app/releases/latest';
  ...
  async function api(path, opts) {
    opts = opts || {};
    opts.headers = Object.assign({ 'X-JAT12-Token': state.token, 'content-type': 'application/json' }, opts.headers || {});
    ...
    for (var i = 0; i < PORTS.length; i++) {
      try {
        ...
        if (body && body.token) {
          ...
          setConn(true, 'app connected' + (PORTS[i] === 7846 ? ' (dev)' : ''));
