...
async function boot(): Promise<void> {
  ...
  console.log(`[jat12] brain on http://127.0.0.1:${server.port}${DEV ? ' (dev)' : ''} · ${registry.all().length} adapter(s)`);
  if (!sealer.available()) console.warn('[jat12] safeStorage unavailable — secrets are NOT encrypted at rest on this machine');
