...
import { parseAdapter } from '@jat12/shared/adapter-schema';
import type { AdapterDoc } from '@jat12/shared/adapter-schema';
...
export function loadBuiltins(dir: string = DEFAULT_BUILTIN_DIR): AdapterDoc[] {
  ...
  try {
    ...
    console.warn(`[jat12] adapter registry: cannot read builtin dir ${dir} — no builtins loaded`, err);
    return [];
  }
  ...
  for (const file of files.sort()) {
    ...
    try {
      ...
      console.warn(`[jat12] adapter registry: skipping malformed adapter ${file}`, err);
    }
  }
